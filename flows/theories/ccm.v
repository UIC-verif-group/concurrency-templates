...
(*
  map_lookup_filter_Some
     : ∀ (P : positive * ?A → Prop) (H7 : ∀ x : positive * ?A, Decision (P x)) 
         (m : pmap.Pmap ?A) (i : positive) (x : ?A),
...
Lemma nzmap_empty_lookup1 `{Countable K} `{CCM A} (m: nzmap K A) : 
  m = ∅ -> ∀ i, m !!! i = 0.
Proof.
  pose proof (nzmap_is_wf m).
  - intros.
