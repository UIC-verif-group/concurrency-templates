...
Module Info.
  ...
  Definition source_file := "coarse.c".
...
Definition f_traverse := {|
  ...
  fn_vars := nil;
  fn_temps := ((_status, tint) :: (_r, (tptr (Tstruct _node noattr))) ::
      ...
      (Sset _t'8
        (Efield
          (Ederef (Etempvar _c (tptr (Tstruct _css noattr)))
            (Tstruct _css noattr)) _lock (tptr (Tstruct _atom_int noattr))))
      (Scall None
    ...
    (Ssequence
      (Ssequence
        ...
        (Sifthenelse (Eunop Onotbool
                       (Etempvar _t'6 (tptr (Tstruct _node noattr))) tint)
          (Ssequence
            (Sset _r
              (Efield
                (Ederef (Etempvar _c (tptr (Tstruct _css noattr)))
...
Definition f_lookupOp_helper := {|
  ...
  fn_params := ((_c, (tptr (Tstruct _css noattr))) ::
                ...
                (_status, tint) :: nil);
  fn_vars := nil;
  fn_temps := ((_v, (tptr tvoid)) :: (_t'1, (tptr tvoid)) ::
               (_t'2, (tptr (Tstruct _atom_int noattr))) :: nil);
  fn_body :=
(Ssequence
  (Sifthenelse (Ebinop Oeq (Etempvar _status tint)
    ...
    (Ssequence
      (Scall (Some _t'2)
        (Evar _makelock (Tfunction nil (tptr (Tstruct _atom_int noattr))
                          cc_default)) nil)
      (Sset _lock (Etempvar _t'2 (tptr (Tstruct _atom_int noattr)))))
    (Ssequence
      (Sassign
        (Efield
...
(Ssequence
  (Scall None
    ...
    ((Evar ___stringlit_1 (tarray tschar 35)) :: nil))
  (Ssequence
    (Ssequence
      (Scall (Some _t'1)
...
(_printDS_helper :: _get_root :: _make_css :: _lookupOp_helper ::
 _insertOp_helper :: _traverse :: _printDS :: _get_value :: _insertOp ::
 _findNext :: _release :: _acquire :: _makelock :: _exit :: _malloc ::
